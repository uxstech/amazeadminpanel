...
if (isset($_GET['fromdate']) && isset($_GET['todate'])) {
    $fromdate = $_GET['fromdate'];
    $todate = $_GET['todate'];
    $result_of_track = mysqli_query($conn, "SELECT SUM(amount_paid) AS value_sum_of_track FROM amd_track_records WHERE registration_date between '$fromdate' AND '$todate' AND created_by ='$branchName'");
} else {
    $result_of_track = mysqli_query($conn, "SELECT SUM(amount_paid) AS value_sum_of_track FROM amd_track_records WHERE created_by ='$branchName'");
}
$row_of_track = mysqli_fetch_assoc($result_of_track);
$sum_of_track = $row_of_track['value_sum_of_track'];

$overall_profit = ($sum_of_training + $sum_of_requests + $sum_of_credited_transactions + $sum_of_track) - ($sum_of_fuel + $sum_of_maintainence + $sum_of_salary + $sum_of_debited_transactions);
